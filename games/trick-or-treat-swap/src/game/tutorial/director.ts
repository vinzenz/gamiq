...
/**
 * DOM shell around the pure `TutorialCore` (ticket ToTS-a5d77f). Re-renders
 * whenever the core's version bumps: a blocking `.tots-overlay` panel for
...
const STYLE_ID = 'tots-tutorial-styles'
...
.tots-tut-toast {
  position: absolute;
  left: 50%;
  bottom: calc(var(--tots-sab, 0px) + 18px);
  ...
  animation: tots-tut-in 0.25s ease;
}
...
.tots-tut-skip:active {
  background: rgb(255 255 255 / 0.12);
}

@keyframes tots-tut-in {
  from {
    opacity: 0;
...
export class TutorialDirector {
  ...
  mount(root: HTMLElement, before?: HTMLElement): void {
    ensureStyles()
    this.#layer = el('div', 'tots-tut-layer')
  ...
  #sync(): void {
    ...
    if (blocking) {
      const overlay = el('div', 'tots-overlay', layer)
      const panel = el('div', 'tots-panel', overlay)
      if (step.title) {
        const title = el('h2', 'tots-panel-title', panel)
        title.textContent = step.title
      }
      const text = el('p', 'tots-panel-subtitle', panel)
      ...
      const toast = el('div', 'tots-tut-toast', layer)
      const card = el('div', 'tots-tut-card', toast)
      if (step.title) {
  ...
  #button(parent: HTMLElement, label: string, primary: boolean, onClick: () => void): void {
    const className = primary ? 'tots-btn tots-primary' : 'tots-tut-skip'
