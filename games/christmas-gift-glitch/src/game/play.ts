...
import { storyFor } from '../meta/story.ts'
...
function describeGoal(goal: GoalDef): string {
  switch (goal.kind) {
    ...
    case 'deliver':
      return `Deliver ${goal.count} ${TILE_LABELS[goal.color]}: clear matches on the bottom row so they drop into the parcel chute.`
    case 'clear-modifier': {
...
const GOAL_HINT_SEEN_KEY = 'christmas-gift-glitch:goal-hint-seen'
...
class PlayScreen implements Screen {
  ...
  resize(width: number, height: number): void {
    ...
    const area = {
      x: inset('--glitch-sal') + 8,
      y: hudBottom + 8,
      w: width - inset('--glitch-sal') - inset('--glitch-sar') - 16,
      h: height - (hudBottom + 8) - inset('--glitch-sab') - 8,
    }
  ...
  #moveStep(left: number | null): Step {
    return {
      ...
      start: () => {
        ...
        if (counter) {
          counter.classList.remove('glitch-pulse')
          void counter.offsetWidth
          counter.classList.add('glitch-pulse')
        }
      },
    }
  }
  ...
  #buildHud(root: HTMLElement): {
    ...
    const hud = el('div', 'glitch-hud', root)
    const top = el('div', 'glitch-hud-top', hud)

    const pause = el('button', 'glitch-icon-btn', top)
    pause.type = 'button'
    pause.textContent = '❚❚'
    ...
    const title = el('div', 'glitch-title', top)
    const name = el('div', 'glitch-level-name', title)
    name.textContent = storyFor(this.#levelIndex).title
    const moves = el('div', 'glitch-moves', title)
    const movesEl = el('span', 'glitch-moves-count', moves)
    movesEl.textContent = String(this.#bundle.level.moves)
    moves.append(' moves left')

    el('span', 'glitch-icon-spacer', top)

    const goalsRow = el('div', 'glitch-goals', hud)
    this.#bundle.level.goals.forEach((goal, index) => {
      this.#chips.push(this.#buildChip(goalsRow, goal, index))
    })
    const goalHint = el('div', 'glitch-goal-hint', hud)
  ...
  #buildChip(row: HTMLElement, goal: GoalDef, index: number): Chip {
    ...
    const chip = el('button', 'glitch-chip', row)
    chip.type = 'button'
    ...
    if (goal.kind === 'collect' || goal.kind === 'deliver') {
      const img = el('img', 'glitch-chip-img', chip)
      img.src = TILE_URLS[goal.color]
      img.alt = goal.color
      img.draggable = false
      if (goal.kind === 'deliver') {
        chip.classList.add('glitch-chip-deliver')
        const arrow = el('span', 'glitch-chip-emoji', chip)
        arrow.textContent = '⬇'
      ...
      const url =
        goal.kind === 'clear-modifier'
          ...
          : goal.kind === 'boss'
            ? this.#bossArtUrl
            : undefined
      const icon = el('span', 'glitch-chip-emoji', chip)
    ...
    const count = el('span', 'glitch-chip-count', chip)
    count.textContent = '0'
    return { chip, count }
  }
  ...
  #updateChips(): void {
    ...
    this.#chips.forEach((chip, i) => {
      ...
      chip.chip.classList.toggle('glitch-done', p.met)
    })
  }
  ...
  #bumpChip(goalIndex: number, delta: number): void {
    ...
    chip.chip.classList.toggle('glitch-done', next >= progress.target)
    chip.chip.classList.remove('glitch-pulse')
    void chip.chip.offsetWidth
    chip.chip.classList.add('glitch-pulse')
  }
  ...
  #pulseChips(): void {
    for (const { chip } of this.#chips) {
      chip.classList.remove('glitch-pulse')
      void chip.offsetWidth
      chip.classList.add('glitch-pulse')
    }
  }
  ...
  #showGoalHint(index: number): void {
    ...
    for (const [i, { chip }] of this.#chips.entries()) {
      chip.classList.toggle('glitch-chip-open', i === index)
    }
    window.clearTimeout(this.#goalHintTimer)
  ...
  #hideGoalHint(): void {
    ...
    for (const { chip } of this.#chips) chip.classList.remove('glitch-chip-open')
    window.clearTimeout(this.#goalHintTimer)
  }
  ...
  #showResult(win: boolean): void {
    ...
    const title = el('h2', 'glitch-panel-title', panel)
    const subtitle = el('p', 'glitch-panel-subtitle', panel)
    const actions = el('div', 'glitch-actions', panel)

    if (win) {
      const stars = levelStars(level, game)
      recordStars(level.id, stars)
      title.textContent = 'Dispatch fixed!'
      title.classList.add('glitch-win')
      const starsRow = el('div', 'glitch-stars', panel)
      for (let i = 0; i < 3; i++) {
      ...
      if (this.#levelIndex + 1 < LEVELS.length) {
        this.#addButton(actions, 'Next dispatch →', true, () =>
      ...
      subtitle.textContent = 'The calendar glitched again. Try once more!'
  ...
  #openOverlay(): HTMLElement {
    ...
    return el('div', 'glitch-panel', this.#overlay)
  }

  #addButton(parent: HTMLElement, label: string, primary: boolean, onClick: () => void): void {
    const button = el('button', primary ? 'glitch-btn glitch-primary' : 'glitch-btn', parent)
    button.type = 'button'
  ...
  #drawBackdrop(ctx: CanvasRenderingContext2D, width: number, height: number): void {
    ...
    gradient.addColorStop(0, '#16455a')
    gradient.addColorStop(0.55, '#0d3046')
    gradient.addColorStop(1, '#081c34')
    ...
    moon.addColorStop(0, 'rgb(124 201 232 / 0.2)')
    moon.addColorStop(1, 'rgb(43 138 110 / 0)')
    ctx.save()
    ...
    for (const star of this.#stars) {
      ...
      ctx.fillStyle = `rgb(255 245 219 / ${twinkle})`
      ctx.beginPath()
