...
import { blockerCell, coverCell, iceCell, lockCell, spreaderCell } from './obstacles.ts'
...
const PALETTE = { a: 'red', b: 'blue', c: 'ivory', d: 'purple', e: 'pink', f: 'green' } as const
...
describe('the ten combo footprints', () => {
  it('sweep + sweep: cross sweep clears the row AND the column', () => {
  ...
  it('sweep + blue: sweeps the densest line, preferring the line through the pair', () => {
  ...
  it('sweep + blast: triple sweep of three rows and three columns', () => {
  ...
  it('sweep + prism: turns the most common plain colour into launched sweeps', () => {
    const board = boardFromRows(['aaab', 'abca', 'baba'], PALETTE)
    requireCell(board, { x: 3, y: 2 }).tile = makeTile('red', 'homing')
    const cells = sweepLaunch({ board, a: { x: 0, y: 0 }, b: { x: 1, y: 0 } })
    deepStrictEqual(posKeys(cells), ['0,0', '0,1', '1,0', '1,2', '2,0', '3,1'])
    for (const [i, at] of cells.entries()) {
      const tile = tileAt(board, at)
      ok(tile)
      strictEqual(tile.powerup, 'sweep')
      strictEqual(tile.dir, i % 2 === 0 ? 'h' : 'v')
    }
    // A tile that already carried a power-up keeps it.
    strictEqual(tileAt(board, { x: 3, y: 2 })?.powerup, 'homing')
  })

  it('blue + blue: three little blues hit three distinct objective tiles', () => {
    const board = boardFromRows(['G.t.', '....', '.t.t'], { G: 'red', t: 'purple' })
    const cells = homingTrio({ board, a: { x: 1, y: 1 }, b: { x: 0, y: 1 } })
    ...
    try {
      const homed = homingTrio({ board, a: { x: 1, y: 1 }, b: { x: 0, y: 1 } })
  ...
  it('blue + blast: drops the red on the densest 3×3 cluster', () => {
    const board = boardFromRows(['aa..d', 'aa.d.', 'aa...'], PALETTE)
    deepStrictEqual(posKeys(redDrop({ board, a: { x: 4, y: 2 }, b: { x: 0, y: 0 } })), [
      '0,0',
      '0,1',
      '0,2',
      '1,0',
    ...
    deepStrictEqual(posKeys(redDrop({ board: uniform, a: { x: 3, y: 0 }, b: { x: 3, y: 1 } })), [
      '1,0',
      '1,1',
      '1,2',
      '2,0',
  ...
  it('blast + blast: giant 5×5 blast centred on the pair, clipped to the board', () => {
  ...
  it('blast + prism: the most common colour chain-detonates as red blasts', () => {
    const board = boardFromRows(['aab', 'acd', 'cba'], PALETTE)
    requireCell(board, { x: 1, y: 2 }).tile = makeTile('purple', 'blast')
    requireCell(board, { x: 2, y: 2 }).tile = makeTile('ivory', 'prism')
    const { plan, combo } = planCombo(board, { x: 1, y: 2 }, { x: 2, y: 2 })
    deepStrictEqual(combo.a, { at: { x: 1, y: 2 }, powerup: 'prism' })
    deepStrictEqual(combo.b, { at: { x: 2, y: 2 }, powerup: 'blast' })
    for (const at of [
      { x: 0, y: 0 },
      { x: 1, y: 0 },
      { x: 0, y: 1 },
    ]) {
      strictEqual(tileAt(board, at)?.powerup, 'blast')
    }
  ...
  it('blue + prism: the most common colour becomes objective-homing little blues', () => {
    const board = boardFromRows(['aab', 'acd', 'cba'], PALETTE)
    requireCell(board, { x: 1, y: 2 }).tile = makeTile('ivory', 'prism')
    requireCell(board, { x: 2, y: 2 }).tile = makeTile('pink', 'homing')
    ...
    try {
      ...
      for (const at of [
        ...
        strictEqual(tileAt(board, at)?.powerup, 'homing')
      }
      const result = resolveMatches(board, createRng(6), TILE_TYPES, [], plan)
      const clear = clearCells(result.events, 0)
      // Cast cells, the three converted blues, and the shared objective tile.
  ...
  it('prism + prism: full-board clear and one layer off every obstacle', () => {
    const board = boardFromRows(['abcde', 'deabc', 'bcdae'], PALETTE)
    board.cells[0] = coverCell(3, makeTile('red'))
    board.cells[1] = blockerCell(2)
    board.cells[2] = iceCell(makeTile('ivory'))
    board.cells[3] = lockCell(makeTile('purple'))
    board.cells[4] = spreaderCell(3)
    requireCell(board, { x: 0, y: 2 }).tile = makeTile('green', 'prism')
    requireCell(board, { x: 1, y: 2 }).tile = makeTile('green', 'prism')
    deepStrictEqual(
    ...
    strictEqual(requireCell(board, { x: 0, y: 0 }).modifier, 'cover-2')
    strictEqual(requireCell(board, { x: 1, y: 0 }).modifier, 'blocker-1')
    ...
    strictEqual(tileAt(board, { x: 0, y: 0 })?.type, 'red')
    const survivors = board.cells.map((cell) => cell.tile?.type)
    ok(survivors.includes('ivory'), 'the cracked-free ice tile should survive')
    ok(survivors.includes('purple'), 'the freed caged tile should survive')
...
describe('combo resolution through the board', () => {
  it('a launched-sweep combo clears the union of the launched sweeps', () => {
    const board = boardFromRows(['aab', 'aca'], PALETTE)
    requireCell(board, { x: 2, y: 0 }).tile = makeTile('purple', 'sweep')
    requireCell(board, { x: 2, y: 1 }).tile = makeTile('green', 'prism')
    const { plan } = planCombo(board, { x: 2, y: 0 }, { x: 2, y: 1 })
    strictEqual(tileAt(board, { x: 0, y: 0 })?.powerup, 'sweep')
  ...
  it('combo sweeps peel exactly one cover layer per direct hit', () => {
    const board = boardFromRows(['abcde', 'deabc', 'bcdae'], PALETTE)
    board.cells[1] = coverCell(2, makeTile('ivory'))
    requireCell(board, { x: 1, y: 1 }).tile = makeTile('red', 'sweep', 'v')
    requireCell(board, { x: 1, y: 2 }).tile = makeTile('purple', 'sweep', 'h')
    const { plan, combo } = planCombo(board, { x: 1, y: 1 }, { x: 1, y: 2 })
    deepStrictEqual(combo, {
      a: { at: { x: 1, y: 1 }, powerup: 'sweep' },
      b: { at: { x: 1, y: 2 }, powerup: 'sweep' },
    })
    ...
    strictEqual(requireCell(board, { x: 1, y: 0 }).modifier, 'cover-1')
    strictEqual(tileAt(board, { x: 1, y: 0 })?.type, 'ivory')
  })
  ...
  it('keeps the default swap behaviour for non-combo pairs', () => {
    ...
    strictEqual(tileAt(board, { x: 1, y: 0 })?.charge, 'blue')

    const plain = boardFromRows(['ab', 'cd'], PALETTE)
    requireCell(plain, { x: 0, y: 0 }).tile = makeTile('red', 'sweep', 'h')
  ...
  it('a combo swap in Swap3Game emits swap → combo → clear and spends one move', () => {
    ...
    requireCell(board, { x: 2, y: 2 }).tile = makeTile('pink', 'homing')
    requireCell(board, { x: 3, y: 2 }).tile = makeTile('blue', 'sweep', 'h')
    ...
    deepStrictEqual(combo.a, { at: { x: 2, y: 2 }, powerup: 'sweep' })
    deepStrictEqual(combo.b, { at: { x: 3, y: 2 }, powerup: 'homing' })

    // The blue carries the sweep to the densest row: row 2, through the pair.
